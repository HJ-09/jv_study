...
getClass()          객체의 클래스 정보를 문자열로 반환 ★
clone()             객체 복제 (주의: Cloneable 인터페이스 필요)
finalize()          가비지 컬렉션(GC) 전에 호출되는 메서드 (거의 안 씀)
...
@Override
void call() { } // 오버라이딩 ⇒ 하나인데 여러개인 것을 "다형성"이라고 부름~
...
🧠 핵심 개념 요약
	...
	•	toString()을 오버라이드하면 객체 정보를 사람이 이해하기 쉽게 만들 수 있다.(★재정의 용도★)
...
>> 정답 : 4
>>> class, for, if, import, extends, "", '', {} 등 ⇒ 자바에서 언어를 구현하기 위한 약속된 표기. 예약어라고 함.
>>> 주의 : 이름으로 쓸 수 없음. ((ex)) int class, int for, String if 등.. 불가능~
⸻
...
>> 정답 : 자식 클래스의 생성자에서 부모 클래스의 해당생성자를 호출해야함.
⸻
...
>> 정답 : 두 객체 필드가 같은지 비교하기 위해 재정의
⸻

10.	부모와 자식 클래스가 동일한 이름의 필드를 가질 경우, this.name과 super.name은 무엇을 의미하나요?
>> 정답 : this.name은 자식 클래스 필드, super.name은 부모 클래스 필드 호출.
⸻
...
class Child extends Parent {
    Child() {
        // 오류 발생 ⇒ super()로 부모 클래스 호출
    }
}

>> 정답 : Parent 클래스에서는 매개변수가 있는 생성자가 정의되었는데 Child 클래스에서는 기본 생성자로 호출해서 오류 발생
>>> 부모 객체를 생성하지 않았기 때문에
⸻
...
>> 정답 : 클래스이름@해시코드(Person@~)
>>> 풀이 : toString()을 오버라이딩하지 않아서..
⸻

⭕ 면접대비 문제 ⭕
17.	오버라이딩(Overriding)과 오버로딩(Overloading)의 차이를 간단히 설명해보세요.
>> 정답 : 오버라이딩은 덮어쓰기, 오버로딩은 같은 이름의 함수에 여러개의 매개변수가 정의되어 있는 것.
>>> 오버로드 : 다형성(1개인데 역할이 여러개인 것)의 일부로 오버로드(여러개)는 1개의 메서드 이름으로 매개변수가 다른 메서드를 생성가능.
>>> 오버라이드 : 부모의 메서드를 자식이 재정의 하는 것. 우선순위 자식이 가져감.
⸻
...
>> 정답 :
>>> 클래스의 기본 설계와 유용한 메서드(toString, equals 등)를 제공합니다.
>>> 모든 객체의 부모라서 모든 객체를 참조할 수 있음. ⇒ 타입의 다형성(유연성)으로 모든 객체를 Object 변수가 참조 가능해서 설계의 유연성을 제공합니다.
⸻
...
>> 정답 : 부모클래스에는 매개변수 생성자가 정의되어 있는데 자식 클래스는 기본 생성자 호출을 해서 오류 발생.
>>> 자식은 부모의 생성자를 "무조건" 호출해야하는데 기본생성자 호출로는 바뀐 부모 생성자를 호출할 수 없기 때문에 명시적으로 부모생성자를 호출해야함.
//  └─ 자식 생성자의 맨 위에서 자식보다 먼저 생성되어야 함. ★
⸻
...
③ 생성자에서 부모의 기본생성자 (super()) 호출은 생략 가능하다.
...
class A{
    int a;
    static int b; // new A()랑 관련 없는 독립적인 데이터 (객체생성 없이 사용가능)

    public void sum(){ //new A()가 될 때 필드로 생성
        ++this.a; //가능
        super.toString();

        this.b; //이렇게 써도 디버거가 코드를 바꿈. A.b로
        ++A.b; //← 이렇게 바꿈
    }

    public static void main(String[]args){ //A와 전혀 관련이 없고 이름만 빌려씀 (객체생성 없이 사용가능)
        ++this.a; //불가능
        super.toString() //불가능
        A a = new A();
        ++a.a; //가능
...
>> 정답 :
>>> 참조(주소)를 비교
>>> 자식 클래스에서 재정의 할 때는 모든 필드가 같은지 비교해야 합니다.
⸻
...
>> 정답 : toString 오버라이딩이 없어서 "자바"라고 출력되지 않음.

class Book {
    String title = "자바";

    @Override
    public String toString(){ //⇒ 문자열로 객체 정보를 출력할게~(그래서 return 값 필요)
    return "책 이름 : "+title; // "Book(title :"+this.title+"}";
    }
}
⸻

27. @Override 어노테이션을 사용하는 이유를 설명하고, 실수로 오타가 날 경우 어떤 장점이 있는지 예시와 함께 설명하세요.
>> 정답 : 부모 클래스에 있는 메서드인지 확인하기 위해서 사용.
오타가 날 경우 컴파일 오류가 뜸.
⸻
...
>>> super()는 상위 객체 호출문, this()는 현재 객체 호출문.
⸻
