...
int [] c={1,2,3,4} // 배열의 리터럴(눈에 보이는 그대로) 표기법 → 기존 데이터 참조 x
...
new double[3]; ==> {0.0,0.0,0.0} 생성
new boolean[2]; ==> {false,false} 생성
new char[1]; ==> {'\u0000'} 생성
new String[2]; ==> {null,null} 생성
...
>> 정답 : int [] nums=new int[3];
        System.out.println(Arrays.toString(nums)); //import 생성
        System.out.println(java.util.Arrays.toString(nums)); //import 없이
...
String[] fruit=new String[4];
        ...
        String fruit2={"사과","딸기","수박","배"} //→리터럴표기
---

### 4. `fruit` 배열의 5번째 요소를 출력하려고 하면 어떤 오류가 발생하며, 왜 발생하는지 설명하세요.
>> 정답 : ArrayIndexOutOfBoundsException 오류가 발생. 인덱스는 0번부터 저장하기 때문에 0~3까지 밖에 없음.(런타임오류/ 컴파일시 발생하는 오류보다 심각한 오류임..!)
---
