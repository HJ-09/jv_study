...
p = new C2();  // 업캐스팅
p.show();      // C2에 오버라이딩이 없다면 역시 P의 show() 실행
...
>> 정답 : C 클래스의 오버라이딩이 되어있지 않으면 P 클래스의 값을 출력함.
⸻
...
if (obj instanceof C c) {   // 검사 + 변수 선언을 한 번에
    c.show();               // 이미 C 타입으로 확정된 변수 c 사용
}
⸻
