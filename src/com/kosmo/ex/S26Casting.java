...
byte b = 127;
Byte bObj = 127; // auto-boxing(자동으로 객체가 된다)
...
2. 다음 중 float와 double의 차이로 인해 발생할 수 있는 문제는?

A. float이 더 정밀하다     ⇒ double이 더 정밀 ^^ (float는 4바이트, double은 8바이트)
...
D. char → int
>> 정답 : C
>>> 기본형은 숫자형변환이라서 런타임오류 발생 x
⸻

/10. 다음 중 형변환 없이 + 연산이 가능한 경우는?

A. int + char
B. boolean + int  ⇒ 자바에서는 불가능
C. float + boolean  ⇒ 자바에서는 불가능
D. byte + byte  ⇒ 정수는 무조건 int로 바뀌어서 더하기 연산이 됨. 글애서 byte 값이 필요하면 형변환 필요.
>> 정답 :
