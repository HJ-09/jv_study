...
사용자가 직접 그림(선, 도형, 이미지, 문자열 등)을 그릴 수 있는 빈 도화지 역할을 하며,
 paint(Graphics g) 또는 update(Graphics g) 메서드를 오버라이드하여 캔버스가 생성될 때 그립니다. (update는 잘 안씀)
 갱신하고 싶을 때 repaint 함수를 호출하면 됨.
...
- Canvas와 일반 컴포넌트(Component)의 차이
	...
	•	JButton, JLabel 등 일반 컴포넌트
        •	이미 정의된 UI 기능을 수행 (버튼 클릭, 텍스트 출력 등) → 수정하기가 곤란
        •	기본 이벤트 처리 포함
        •	자동으로 UI 요소가 그려짐 → 배치매니저에 의해서 UI가 정해진다는 말.
...
⸻
    ...
    g.drawRect(int x, int y, int width, int height) → 비어있는 사각형(테두리만 존재)
    g.fillRect(int x, int y, int width, int height) → 꽉 차 있는 사각형
    ...
    g.drawImage(Image img, int x, int y, ImageObserver observer)
        ...
        //같은 이미지를 paint()에서 계속 불러오면 안됨! 필드로 정의핫세요!
        Image img = Toolkit.getDefaultToolkit().getImage("player.png");
        g.drawImage(img, 50, 50, this);

        이미지가 비동기적으로 생성되고 paint가 호출될 수 있으므로 ImageObserver를 활용하여 로딩 상태를 관리해야 정확함.
        ***비동기적(=멀티스레드) → 정보를 일정한 속도로 보낼 것을 요구하지 않는 데이터 전송 방법.
        mageObserver는 자바에서 **이미지 로딩 상태를 감시(관찰)**하기 위해 사용하는 인터페이스입니다.
        자바의 drawImage()는 이미지가 **비동기적(Async)**으로 로딩되는 방식이라,
        아직 이미지가 완전히 메모리에 로드되지 않았을 때도 drawImage()가 호출될 수 있습니다.
        이때, ImageObserver를 통해 그 진행 상황을 감지하고 업데이트할 수 있습니다.
