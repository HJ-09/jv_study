...
} catch (FileNotFindException e) { //파일을 찾았는데 없을 때
    System.out.println("파일을 못찾았습니다.");
} catch (IOException e) { //통신 중일 때(통신 중에 끊어졌을 때, 인터넷 주소가 잘못 되었을 때 등)
    System.out.println("통신을 확인하세요.");
} catch (Exception e) { //예외의 원인 모를 때. 제일 마지막에 써야 함. 마치 else if처럼
