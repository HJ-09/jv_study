...
•	레이아웃 매니저
    ...
    대표적인 레이아웃 매니저
    •	FlowLayout:
        컴포넌트를 왼쪽에서 오른쪽으로 순서대로 나열 (넣은 순서대로. 가로길이가 중요함)
...
⸻
	...
	•	ActionListener: 버튼 클릭, 키보드 엔터, 메뉴 클릭 등 클릭 이벤트 (글씨를 쓰고 엔터를 치면 액션리스너 발동!)
...
- Character 전용 메서드
	•	isDigit(char)
        숫자인지 확인 (⇒ 'A' , '1' ... 이게 숫자인지 문자인지 구분하는 칭구)
	•	isLetter(char)
        알파벳인지 확인
...
Integer i = 10;        // auto-boxing(기본형→자료형)
int n = i;             // auto-unboxing(자료형→기본형)
