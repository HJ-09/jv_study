...
- 비유로 이해하기
	•	스트림은 데이터가 흐르는 “수도관”
	•	입력 스트림 : 수돗물을 틀어 들여오는 것. 꼭 close 필요!
	•	출력 스트림 : 물을 밖으로 흘려보내는 것
...
- 스트림의 핵심 특징 요약
	...
	•	다양한 출처와 목적지 지원: 파일, 네트워크, 메모리, 콘솔 등 모든 데이터를 처리할 수 있음.
...
2. 스트림의 종류
	...
	•	데이터 단위 기준
        ...
        •	문자 스트림 (2바이트 단위) ⇒ 자바가 UTF-16 2byte 고정이라서.
            •	Reader, Writer 계열
            •	주로 텍스트 데이터 처리
...
3. 주요 클래스 구조
    ...
    •	FileInputStream, FileOutputStream : 파일에 문자열을 바이트 단위로 저장 및 읽기
    ...
    •	FileReader, FileWriter : 파일에 문자열을 2바이트 단위로 저장 및 읽기
    •	BufferedReader, BufferedWriter : 버퍼로 한줄씩(readLine) 읽고 쓰기 가능 (스트링 버퍼랑 비슷)
